...
def register_teacher(teacher_username, teacher_name, teacher_password, teacher_confirm_password):
    ...
    try:
        ...
        return False, f"Unexpected error occurred while creating teacher profile: {str(e)}"
...
def teacher_screen_login():
    ...
    st.header("Login Using Your Teacher Profile")
    ...
    with btn1:
        if st.button("Login", key="login", type="secondary", icon=":material/arrow_forward:", icon_position="right", width='stretch'):
    ...
    with btn2:
        if st.button("Register Instead", type="secondary", key="register", icon=":material/arrow_forward:", icon_position="right", width='stretch', on_click=lambda: st.session_state.update(teacher_login_type='register')):
            teacher_screen_register()
            st.rerun()
...
def teacher_screen_register():
    ...
    with col1:
        header_dashboard()
    with col2:
        st.button("Go Back To Home", key="go_back2", type="primary", icon=":material/arrow_back:", icon_position="left", on_click=lambda: st.session_state.update(login_type=None))
    ...
    with btn1:
        if st.button("Register Now",  type="secondary", icon=":material/arrow_forward:", icon_position="right", width='stretch'):
            success,message = register_teacher(teacher_username, teacher_name, teacher_password, teacher_confirm_password)
            if success:
                st.success(message)
                import time 
                time.sleep(2)
                st.session_state.update(teacher_login_type='login')
                st.rerun()
            else:
                st.error(message)

    with btn2:
        st.button("Login Instead", type="secondary",     icon=":material/arrow_forward:", icon_position="right", width='stretch', on_click=lambda: st.session_state.update(teacher_login_type='login'))
...
def teacher_dashboard():
    ...
    with c2:
        ...
        if st.button("Logout", key="go_back2", type="primary",shortcut="control+backspace"):
...
def teacher_tab_take_attendance():
    ...
    col1, col2 = st.columns([3,1], vertical_alignment='bottom', gap='xxsmall')

    with col1:
        selected_subject_label = st.selectbox('Select Subject', options=list(subject_options.keys()), index=0, help='Select the subject for which you want to take attendance')
